...
#[tokio::test]
async fn success_migration() {
    let (mut context, test_pool_market, test_pool, _pool_borrow_authority, _) = setup().await;

    let (_, withdraw_requests_deprecated) = test_pool
        .get_withdrawal_requests_deprecated(&mut context, &test_pool_market)
        .await;

    test_pool
        .migrate_withdraw_requests_account(&mut context, &test_pool_market)
        .await
        .unwrap();
    ...
    assert_eq!(withdraw_requests_deprecated.mint, withdraw_requests.mint);

    assert_eq!(withdraw_requests_deprecated.pool, withdraw_requests.pool);
}
