...
async fn success_with_sol() {
    let (mut context, test_registry, test_pool_market, test_pool, _pool_borrow_authority, user, mining_acc) =
...
async fn success() {
    ...
    test_pool
        .withdraw_request(&mut context, &test_registry, &test_pool_market, &user, mining_acc, 45)
        .await
        .unwrap();
